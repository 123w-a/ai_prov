...
STRUCTURE_PROMPT = ChatPromptTemplate.from_messages([#专门的对话式提示词
    (#按照我定义的规则定义的最终提示词
        ...
        "8. 若上下文包含同餐家庭成员信息且用户没有明确要求多道菜，只输出一道全家共同主菜；"
        "在 member_adjustments 中给其他成员各写一句话调整建议（如少盐、控糖、切碎、减份量），"
        "不要为任何成员生成第二份完整菜谱；其他成员的过敏原或慢病不要扩展成全家禁菜，"
        "应在调整建议中标明单独替换或单独特制。有可靠的菜品可用性判断时同步填入 dish_matrix，拿不准就留空。\n"
