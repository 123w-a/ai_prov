"""RAG 召回质量测试（纯本地检索，不调 LLM，确定性、秒级）。

验证nutrition_rules 护栏的"知识来源"——权威健康知识库能否被准确召回、可溯源。
这是护栏端到端生效的前提：verify_answer 审计时依赖 nutrition_kb_search 召回的正确规则。
"""
...
class TestRagRecall(unittest.TestCase):
    def test_chronic_disease_query_returns_authoritative_doc(self):
        """慢病查询应召回权威指南/食养文档（护栏可溯源的前提）。"""
        r = kb_search("高血压 低盐 饮食 原则 怎么吃", n_results=3)
    ...
    def test_gout_forbidden_food_query_recalls_gout_guide(self):
        """痛风/高嘌呤忌口查询应召回对应食养指南，且命中带 source+section（可溯源）。"""
        r = kb_search("痛风 忌口 高嘌呤 黄豆 猪肝 老火汤", n_results=3)
        self.assertFalse(r.error, msg=f"检索报错: {r.error}")
    ...
    def test_tool_nutrition_kb_search_json_shape(self):
        ...
        if hasattr(fn, "invoke"):
            raw = fn.invoke({"query": "糖尿病 控糖 主食 怎么吃", "n_results": 3, "source_filter": ""})
        else:
            raw = fn("糖尿病 控糖 主食 怎么吃", n_results=3)
        out = json.loads(raw)
